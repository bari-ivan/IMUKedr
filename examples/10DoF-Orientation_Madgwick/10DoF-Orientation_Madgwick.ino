...
// Калибровочные данные для магнетрометра
const double compassCalibrationBias[3] = { 556.345, -739.567, 922.619 };
 
const double compassCalibrationMatrix[3][3] = { { 2.11,  0.092,   0.01 },
                                               { 0.001, 1.736,  -0.341 },
                                               { -0.001, 0.012, 2.098} };
...
void setup() {
  ...
  // Калибровка компаса
  kedr.setMagnetrometerCalibration(compassCalibrationMatrix, compassCalibrationBias);

  // Инициализация модуля
  kedr.IMUInit(KDR_IMU_MADGWICK_MAG, KDR_IMU_FAST);
